...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.cancelButton.backgroundColor = [UIColor colorWithWhite:0.5 alpha:0.3];
        ...
        self.tableViewUnderView.backgroundColor = [UIColor colorWithRed:237/255.0 green:237/255.0 blue:237/255.0 alpha:1];
...
-(void)creatDropView:(UIView *)view withShowTableNum:(NSInteger)tableNum withData:(NSArray *)arr{
    ...
    if (!self.show) {
        ...
        if(arr.count== 3){
            self.tableViewUnderView.frame = CGRectMake(0, 0, self.frame.size.width, self.rowHeightNum * 3);
        }else{
            self.tableViewUnderView.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
        }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    XPDropCell *cell = [tableView dequeueReusableCellWithIdentifier:@"DropCell" forIndexPath:indexPath];
...
- (void)adjustCellFont:(UITableViewCell *)cell{
    if (cell.textLabel.text.length >8){
        [cell.textLabel setFont:[UIFont systemFontOfSize:11]];
    }else if (cell.textLabel.text.length >6){
        [cell.textLabel setFont:[UIFont systemFontOfSize:13]];
    }else if (cell.textLabel.text.length >4){
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.tableCount == 1) {
        ...
        [_delegate dropMenuView:self didSelectName:self.dataArr[indexPath.row][@"label"] fullAddressName:@""];
        ...
        if (tableView == self.tableViewArr[0]) {
            ...
            [self dismiss];
             ...
             [_delegate dropMenuView:self didSelectName:self.dataArr[firstSelectRow][@"subcategories"][indexPath.row] fullAddressName:@""];
        }
        
    }else if (self.tableCount == 3){
        XPDropCell *cell = [tableView cellForRowAtIndexPath:indexPath];
        NSInteger firstSelectRow = ((UITableView *)self.tableViewArr[0]).indexPathForSelectedRow.row;
        NSInteger secondSelectRow = ((UITableView *)self.tableViewArr[1]).indexPathForSelectedRow.row;
        ...
        if (tableView == self.tableViewArr[0]) {
            ...
            if ([cell.textLabel.text isEqualToString:@"全部"]){
                [self saveSelects];
                [self dismiss];
                [_delegate dropMenuView:self didSelectName:self.dataArr[firstSelectRow][@"name"] fullAddressName:self.dataArr[firstSelectRow][@"name"]];
            }else{
                [self adjustTableViews];
                [thirdTableView reloadData];
            ...
            NSString *selectedAddressTitle;
            NSString *provinceTitle = self.dataArr[firstSelectRow][@"name"];
            if ([cell.textLabel.text isEqualToString:@"全部"]){
                selectedAddressTitle =self.dataArr[firstSelectRow][@"sub"][secondSelectRow][@"name"];
                fullAddress = [provinceTitle stringByAppendingString:selectedAddressTitle];
            }else{
                selectedAddressTitle = self.dataArr[firstSelectRow][@"sub"][secondSelectRow][@"sub"][indexPath.row];
                fullAddress = [[provinceTitle stringByAppendingString:self.dataArr[firstSelectRow][@"sub"][secondSelectRow][@"name"]]stringByAppendingString:selectedAddressTitle];
            }
            [_delegate dropMenuView:self didSelectName:selectedAddressTitle fullAddressName:fullAddress];
...
-(void)saveSelects{
    ...
    [self.tableViewArr enumerateObjectsUsingBlock:^(UITableView *tableView, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        self->selects[idx] = tableView.superview ? tableView.indexPathForSelectedRow.row : -1;
    }];
}
...
-(NSArray *)tableViewArr{
    ...
    if (_tableViewArr == nil) {
        ...
        for (UITableView *tableView in _tableViewArr) {
            ...
            [tableView registerClass:[XPDropCell class] forCellReuseIdentifier:@"DropCell"];
